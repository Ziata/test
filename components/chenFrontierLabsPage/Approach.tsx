...
export default function Approach(
    ...
    return (
        <>
            <div
                className={["inline-block cursor-pointer relative flex justify-center  flex-row sm:flex-col w-full sm:w-40", 'approach-dot',
                    className].join(' ')}
                onMouseEnter={() => onMouseEnter(active)}
                onMouseLeave={() => onMouseLeave(active)}>
                <div
                    className={['transition-all mx-auto self-center hidden sm:block absolute -top-[55%]', opacityClass].join(' ')}>

                    {
                        isActive && (<div className={['animate__animated animate__faster mt-2', dropClass].join(' ')}>
                            <section className="text-center text-neutral-500"><h2 className="pb-3"><span
                                className="text-tcci-blue font-normal text-xl" dangerouslySetInnerHTML={{__html:title}}></span></h2></section>
                        </div>)
                    }
                ...
                {isActive && (<div
                    className={['transition-all mx-auto self-center hidden sm:block absolute top-full w-[180px]', opacityClass].join(' ')}>

                    <div className={['animate__animated animate__faster mt-2', DownDropClass].join(' ')}>

                        <section className="text-center text-neutral-500"><p
                            className="text-neutral-500 text-sm" dangerouslySetInnerHTML={{__html:content}}></p></section>
                    </div>
